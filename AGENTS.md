...
- **Adding features that touch invariants**: When adding a new feature that introduces state (config fields, metadata fields, managed files, remotes), consider whether `wsp doctor` should validate it. Doctor checks live in `src/cli/doctor.rs` as standalone helper functions (`check_*`). Each check emits a `DoctorCheck` with scope, status, fixable flag, and optional details JSON. Fixable checks must handle both `--fix` and report-only modes. New check types automatically appear in `--json` output.

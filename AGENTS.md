...
- **Adding skills**: New skills in `skills/` need a corresponding `include_str!` constant in `src/agentmd.rs` and must be wired into `install_skill()` to be installed into workspaces. Also add the skill to the `managed` and `managed_dirs` sets in `check_claude_dir()` in `src/workspace.rs` — otherwise `wsp rm`/`wsp rename` will flag the skill directory as user content and block removal. Run `/check-skill-registration` to verify.

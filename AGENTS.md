...
- **Adding skills**: New skills in `skills/` need a `const SKILL_CONTENT: &str = include_str!(...)` in `src/agentmd.rs` and must be wired into `install_skill()` to be installed into workspaces. The skill is also installed globally via `wsp setup skill install`.

...
- **Adding fields to `Config`**: The `Config` struct uses `#[derive(Default)]` for production code. Search for `Config {` across the codebase when adding new fields. Also update `src/cli/cfg.rs` (`run_list`, `run_get`, `run_set`, `run_unset`) to handle the new config key, add it to `complete_config_keys()` in `src/cli/completers.rs` (with value completions in `complete_config_values()` if applicable), and document it in the `config` help topic in `src/cli/help.rs`.
...
- **Adding fields to output structs**: `RepoStatusEntry`, `RepoDiffEntry`, `RepoLogEntry`, `ExecRepoResult`, `SyncRepoResult`, `SyncAbortRepoResult`, and `MutationOutput` have constructors spread across CLI files (`status.rs`, `diff.rs`, `log.rs`, `exec.rs`, `sync.rs`, `new.rs`, `rename.rs`, `delete.rs`, etc.), codegen `sample()` methods in `src/output.rs`, and 5-15 test sites in `src/output.rs`. Search for `StructName {` across the codebase when adding new fields. After changes, run `just skill` to regenerate SKILL.md.
...
- **Interactive prompts and Ctrl-C**: `read_stdin_line()` in `src/util.rs` returns `"\n"` when the user presses Enter but `""` (empty, no newline) on EOF or read error (e.g. SIGINT/Ctrl-C). Code that prompts interactively must distinguish these: `.trim()` makes both look identical. Use `setup.rs:read_prompt()` as the pattern — check for empty string before trimming and bail to let the SIGINT handler in `main.rs` exit with code 130. Every interactive command should also gate on `std::io::stdin().is_terminal()` and provide a non-interactive fallback.
...
- **Adding experimental features**: New unstable features go behind the `experimental` config gate in `src/config.rs`. Add the feature name to `EXPERIMENTAL_FEATURES` const, then check with `cfg.experimental.as_ref().is_some_and(|e| e.is_feature_enabled("feature-name"))`. Features can be bool-valued (`ExperimentalValue::Bool`) or string-valued (`ExperimentalValue::String`) — string-valued features (like `shell-tmux: window-title`) need a custom match arm in `cfg.rs` `run_set` with value validation, while bool features use the generic handler. The `cfg.rs` handlers for `experimental.*` keys are generic for bools — no per-feature wiring needed there. Setting a specific feature auto-enables the top-level gate. When a feature graduates, remove it from `EXPERIMENTAL_FEATURES` and move its config to a top-level field.
- **Adding features that touch invariants**: When adding a new feature that introduces state (config fields, metadata fields, managed files, remotes), consider whether `wsp doctor` should validate it. Doctor checks live in `src/cli/doctor.rs` as standalone helper functions (`check_*`). Each check emits a `DoctorCheck` with scope, status, fixable flag, and optional details JSON. Fixable checks must handle both `--fix` and report-only modes. New check types automatically appear in `--json` output. **Design rule: every Warn/Error check must either be auto-fixable (`fixable: true`) or include actionable guidance in the message telling the user exactly what command to run.** Warnings without a path to resolution are noise — downgrade to Ok (informational) or don't emit them.

...
- **Interactive prompts**: `read_stdin_line()` in `src/util.rs` returns `"\n"` when the user presses Enter but `""` (empty, no newline) on EOF or pipe close. Ctrl-C is handled globally (`process::exit(130)` in the ctrlc handler), but interactive commands should still detect EOF by checking for empty string before trimming — see `setup.rs:read_prompt()` as the pattern. Every interactive command should gate on `std::io::stdin().is_terminal()` and provide a non-interactive fallback.
